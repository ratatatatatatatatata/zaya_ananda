    ...
    <div className="mx-auto max-w-3xl">
      ...
      {payPlan && (
        <div className="fixed inset-0 z-[60] grid place-items-center bg-black/60 p-4 backdrop-blur-sm" onClick={() => setPayPlan(null)}>
          <div className="w-full max-w-sm rounded-3xl bg-surface-1 p-6 text-center shadow-lift" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mx-auto mt-4 grid h-44 w-44 grid-cols-9 gap-0.5 rounded-2xl border border-line bg-surface-1 p-2">
              {Array.from({ length: 81 }).map((_, i) => <div key={i} className={(fnv(payPlan + "qr" + i) % 10 < 5) ? "bg-ink" : "bg-surface-1"} />)}
            </div>
