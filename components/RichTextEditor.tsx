...
export function RichTextEditor({ value, onChange, minHeight = 180 }: { value: string; onChange: (html: string) => void; minHeight?: number }) {
  ...
  const Btn = ({ cmd, arg, children, title }: { cmd: string; arg?: string; children: React.ReactNode; title: string }) => (
    <button type="button" title={title} onMouseDown={(e) => e.preventDefault()} onClick={() => exec(cmd, arg)}
      className="grid h-8 min-w-8 place-items-center rounded-md border border-line bg-white px-1.5 text-sm text-ink/80 hover:bg-primary-50">
      {children}
    </button>
  );
  ...
  return (
    <div className="rounded-xl border border-line bg-white">
      <div className="flex flex-wrap items-center gap-1.5 border-b border-line bg-aqua/60 p-2">
        <select className="h-8 rounded-md border border-line bg-white px-1 text-sm" defaultValue="" title="Фонт"
          onChange={(e) => { if (e.target.value) exec("fontName", e.target.value); }}>
          {FONTS.map((f) => <option key={f.label} value={f.value}>{f.label}</option>)}
        </select>
        <select className="h-8 rounded-md border border-line bg-white px-1 text-sm" defaultValue="3" title="Хэмжээ"
        ...
        <label className="grid h-8 w-8 cursor-pointer place-items-center rounded-md border border-line bg-white" title="Үсгийн өнгө">
