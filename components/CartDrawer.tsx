...
export function CartDrawer() {
  const { items, isOpen, close, total, count, remove, setQty, continueBrowsing } = useCart();
  const { t, tr } = useI18n();
  ...
  useEffect(() => {
    ...
    const position = { top: window.scrollY, left: window.scrollX, behavior: "instant" as ScrollBehavior };
    const path = window.location.pathname;
    element?.showModal();
    window.scrollTo(position);
    return () => {
      element?.close();
      if (window.location.pathname === path) trigger?.focus({ preventScroll: true });
    };
  }, [isOpen]);

  if (!isOpen) return null;
  return createPortal(
      <dialog ref={dialog} data-cart-drawer className={cx(styles.drawer, "bg-cream text-ink shadow-2xl")}
