        ...
        {COURSE_LEVELS.map((l) => {
          ...
          return (
            <button
              ...
              className={
                "panel group relative flex h-full flex-col overflow-hidden p-7 text-left transition " +
                (active ? "ring-2 ring-primary-500 -translate-y-1 shadow-glow" : "hover:-translate-y-1 hover:shadow-sm")
              }
            >
          ...
          {open && (byLevel[open]?.length ?? 0) === 0 ? (
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {open && byLevel[open].map((c) => (
                <Link key={c.id} href={"/item/" + c.id} className="card group flex h-full flex-col overflow-hidden transition hover:-translate-y-1 hover:shadow-glow">
