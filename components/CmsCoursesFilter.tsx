...
export function CmsCoursesFilter({ items }: { items: CmsItem[] }) {
  ...
  return (
    <div>
      <div className="mx-auto mb-6 flex w-full max-w-md rounded-full border border-line bg-white p-1 shadow-sm">
        {TABS.map((tb) => (
      ...
      <div className="mb-8 flex flex-wrap justify-center gap-2">
        {["Бүгд", ...COURSE_CATS].map((c) => (
          <button key={c} onClick={() => setCat(c)} className={cx("rounded-full px-5 py-2 text-sm font-semibold transition", cat === c ? "bg-primary-grad text-white shadow-glow" : "border border-line bg-white text-ink/70 hover:border-primary-300")}>
