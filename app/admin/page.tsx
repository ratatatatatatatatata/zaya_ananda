...
export default function AdminPage() {
  ...
  async function toggleAdmin(id: string, name: string, makeAdmin: boolean) {
    const q = makeAdmin ? `"${name}"-д админ эрх олгох уу? Тэр бүх контент, захиалгыг удирдах боломжтой болно.` : `"${name}"-ийн админ эрхийг хасах уу?`;
    if (!confirm(q)) return;
    const r = await fetch("/api/admin/users", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, isAdmin: makeAdmin }) });
    if (r.ok) reloadData();
  ...
  if (!user) {
    return <div className="section"><div className="container-px"><div className="mx-auto max-w-md rounded-4xl border border-line bg-[#1A2742] p-10 text-center shadow-card">
    ...
    <div className="bg-transparent">
      <div className="container-px py-8">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[230px_1fr]">
          <aside className="h-fit rounded-3xl border border-line bg-[#1A2742] p-2 lg:sticky lg:top-24">
            {nav.map((n) => (
            ...
            {tab === "users" && (
              <div className="card overflow-x-auto">
                <table className="w-full min-w-[560px]">
                  <thead className="border-b border-line bg-aqua"><tr><Th>{t("admin.colName")}</Th><Th>{t("admin.colEmail")}</Th><Th>{t("form.phone")}</Th><Th>{t("admin.colDate")}</Th><Th>Эрх</Th><Th>Үйлдэл</Th></tr></thead>
                  <tbody>
                    {(data?.users ?? []).map((u) => (
                      <tr key={u.id} className="border-b border-line last:border-0">
                        <Td className="font-medium text-ink">{u.name}</Td><Td>{u.email}</Td><Td>{u.phone || "—"}</Td><Td>{u.createdAt.slice(0, 10)}</Td>
                        <Td>{u.isAdmin ? <span className="rounded-full bg-primary-500/15 px-2.5 py-1 text-xs font-semibold text-primary-300">Админ</span> : <span className="text-xs text-muted">Хэрэглэгч</span>}</Td>
                        <Td><div className="flex flex-wrap gap-2">
                          <button onClick={() => resetUserPassword(u.id, u.name)} className="text-sm font-semibold text-primary-700 hover:underline">Нууц үг солих</button>
                          {user?.isSuper && u.id !== user.id && (
                            u.isAdmin
                              ? <button onClick={() => toggleAdmin(u.id, u.name, false)} className="text-sm font-semibold text-rose-500 hover:underline">Админ хасах</button>
                              : <button onClick={() => toggleAdmin(u.id, u.name, true)} className="text-sm font-semibold text-jade-600 hover:underline">Админ болгох</button>
                          )}
                        </div></Td>
                      </tr>
                    ))}
